...
interface BatchToPettyCashConversionDialogProps {
    open: boolean;
...
export function BatchToPettyCashConversionDialog({
    open,
    onOpenChange,
    payment,
    batchId,
    batchReference,
    onSuccess
}: BatchToPettyCashConversionDialogProps) {
    ...
    const handleConvert = async () => {
        ...
        try {
            const result = await PaymentConversionService.convertBatchToPettyCash(
                payment.id,
                batchId
            );

            if (result.success) {
                toast.success('Payment converted to petty cash successfully');
                onSuccess();
                onOpenChange(false);
                setConfirmStep(1);
            } else {
            ...
            <DialogContent className="max-w-2xl">
                <DialogHeader>
                    <DialogTitle className="flex items-center gap-2">
                        ...
                        Convert to Petty Cash
                    </DialogTitle>
                    <DialogDescription>
                        {confirmStep === 1
                            ? 'This will remove the payment from the batch and move it to petty cash flow.'
                ...
                <div className="space-y-4 py-4">
                            ...
                            <div className="flex items-start gap-3">
                                ...
                                <div>
                                    ...
                                    <p className="text-sm text-destructive/80 mt-1">
                                        The payment will be removed from batch {batchReference} and will require separate execution as petty cash.
                                    </p>
                                </div>
                            </div>
                        </div>
                    ...
                    <div className="bg-blue-50 dark:bg-blue-950/20 p-4 rounded-lg border border-blue-200 dark:border-blue-800">
                        ...
                        <ul className="text-sm text-blue-700 dark:text-blue-300 space-y-1">
                            <li>• Payment moved to Petty Cash tab</li>
                            <li>• "Converted from Batch" tag applied</li>
                            <li>• Ready for immediate cash payout execution</li>
                            <li>• Receipt upload required after payment</li>
                        </ul>
                    </div>
                </div>
