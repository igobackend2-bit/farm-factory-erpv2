...
export default function AdminAuditorPaymentAuditPage() {
    ...
    const handleVerify = async (id: string) => {
        const result = await updateStatus(id, 'ceo_audit');
        if (result.success) {
            toast.success('Payment approved by Admin (Auditor Override) and sent to CEO');
    ...
    const handleReject = async (id: string) => {
        ...
        if (result.success) {
            toast.success('Payment rejected by Admin (Auditor Override)');
            setSelectedPayment(null);
            setRejectionReason('');
        }
    };
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
                ...
                <p className="text-muted-foreground animate-pulse font-medium">Loading Auditor Queue (Admin View)...</p>
            </div>
        );
    }
    ...
    return (
        ...
        >
                ...
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-5">
                        <div className="p-4 rounded-2xl bg-authority-admin/10 border border-authority-admin/20 shadow-inner">
                            <ShieldAlert className="w-8 h-8 text-authority-admin" />
                        </div>
                        <div>
                            <h1 className="text-3xl font-black tracking-tight text-foreground">Auditor Payment Audit</h1>
                            <p className="text-muted-foreground font-medium flex items-center gap-2">
                                Administrative Override <span className="w-1.5 h-1.5 rounded-full bg-authority-admin animate-pulse" /> Admin Mode
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4 bg-background/50 backdrop-blur-md p-4 rounded-2xl border border-border shadow-sm">
                        ...
                        <div className="px-4">
                            ...
                            <Badge variant="outline" className="bg-authority-admin/10 text-authority-admin border-authority-admin/30 font-bold uppercase">Override Active</Badge>
                        </div>
                    </div>
                </div>
            </div>
            ...
            <motion.div
                ...
                className="flex items-center gap-4 p-4 rounded-2xl bg-amber-500/5 border border-amber-500/10 text-amber-600 shadow-sm"
            >
                <AlertCircle className="w-5 h-5 flex-shrink-0" />
                <p className="text-sm font-semibold truncate">Admin Override: Only use this if the National Head Auditor is unavailable to prevent workflow bottlenecks.</p>
            </motion.div>
            ...
            <div className="grid gap-6">
                                ...
                                >
                                    ...
                                    <div className="space-y-4 pt-4 border-t border-border/50">
                                        ...
                                        <div className="flex flex-col sm:flex-row gap-3">
                                            ...
                                            >
                                                <Check className="w-5 h-5 mr-2" /> ADMIN OVERRIDE & APPROVE TO CEO
                                            </Button>
                                            <Button
                ...
                {requests.length === 0 && (
                    ...
                    >
                        ...
                        <div>
                            <h3 className="text-xl font-bold text-foreground/80">Auditor Queue Clear</h3>
                            <p className="text-muted-foreground max-w-xs mx-auto">No payments are currently pending auditor verification.</p>
                        </div>
                    </motion.div>
                )}
            </div>
