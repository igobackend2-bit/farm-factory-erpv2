...
export default function HRPaymentAuditPage() {
    const { requests, isLoading, refresh: refetch } = useRealtimePayments(['hr_audit']);
    ...
    if (isLoading) {
        return (
            <div className="flex flex-col items-center justify-center min-h-[400px] space-y-4">
                ...
                <p className="text-muted-foreground animate-pulse font-medium">Loading HR Audit queue...</p>
            </div>
        );
    }
    ...
    return (
        ...
        >
            {/* Header Section */}
            <div className="relative overflow-hidden rounded-3xl bg-gradient-to-br from-authority-hr/15 via-background to-background p-8 border border-authority-hr/20 shadow-xl">
                <div className="absolute top-0 right-0 -m-12 w-64 h-64 bg-authority-hr/10 rounded-full blur-3xl" />
                <div className="relative z-10 flex flex-col md:flex-row items-center justify-between gap-6">
                    <div className="flex items-center gap-5">
                        <div className="p-4 rounded-2xl bg-authority-hr/10 border border-authority-hr/20 shadow-inner">
                            <Users className="w-8 h-8 text-authority-hr" />
                        </div>
                        <div>
                            <h1 className="text-3xl font-black tracking-tight text-foreground">HR Payment Audit</h1>
                            <p className="text-muted-foreground font-medium flex items-center gap-2">
                                Salary Advance Verification <span className="w-1.5 h-1.5 rounded-full bg-authority-hr animate-pulse" /> HR Mode
                            </p>
                        </div>
                    </div>
                    <div className="flex items-center gap-4 bg-background/50 backdrop-blur-md p-4 rounded-2xl border border-border shadow-sm">
                        <div className="text-center px-4 border-r">
                            ...
                            <p className="text-2xl font-black text-authority-hr">{requests.length}</p>
                        </div>
                        <div className="px-4">
                            <p className="text-[10px] font-bold text-muted-foreground uppercase tracking-wider mb-1">Status</p>
                            <Badge variant="outline" className="bg-authority-hr/10 text-authority-hr border-authority-hr/30 font-bold uppercase">Active</Badge>
                        </div>
                    </div>
                </div>
            </div>
            ...
            >
                ...
                <p className="text-sm font-semibold truncate">Verify employee eligibility and existing balance for salary advance requests before approving.</p>
            </motion.div>
            ...
            <div className="grid gap-6">
                {requests.map((payment, index) => {
                    ...
                    return (
                        <motion.div
                            ...
                            className={cn(
                                'group relative overflow-hidden rounded-2xl bg-card border border-border/50 hover:border-authority-hr/40 transition-all duration-300 shadow-sm hover:shadow-xl',
                                isExpanded && 'ring-2 ring-authority-hr/30 border-authority-hr/50'
                            )}
                        >
                            ...
                            >
                                <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
                                    <div className="flex items-start gap-5">
                                        ...
                                        <div>
                                            <h3 className="text-lg font-bold group-hover:text-authority-hr transition-colors">{payment.purpose}</h3>
                                    ...
                                    <div className="flex items-center gap-8">
                                        <div className="text-right">
                                            ...
                                            <p className="text-2xl font-black flex items-center justify-end text-authority-hr">
                            ...
                            {isExpanded && (
                                ...
                                >
                                    <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
                                        ...
                                        <div className="p-4 rounded-xl bg-background border shadow-sm">
                                            ...
                                            <div className="space-y-2">
                                                ...
                                                <div className="flex items-center justify-between">
                                                    ...
                                                    <Badge variant="outline" className="text-[10px] uppercase font-bold border-authority-hr/40 text-authority-hr">{payment.department?.replace('_', ' ')}</Badge>
                                                </div>
                                            </div>
                                        </div>
                                        ...
                                        <div className="p-4 rounded-xl bg-background border shadow-sm">
                                            ...
                                            <div className="space-y-3">
                                                ...
                                                <div className="flex items-center gap-3">
                                                    <Clock className="w-4 h-4 text-authority-hr" />
                                                    <div>
                                                        <p className="text-xs font-bold leading-none">{format(new Date(payment.cutoff_date), 'dd MMM yyyy')}</p>
                                                        <p className="text-[10px] text-authority-hr">Cut-off Target</p>
                                                    </div>
                                                </div>
                                            </div>
                                        </div>
                                    ...
                                    <div className="p-5 rounded-2xl bg-authority-hr/5 border border-authority-hr/15 shadow-inner">
                                        <h4 className="text-xs font-black text-authority-hr uppercase tracking-[0.2em] mb-4 flex items-center gap-2">
                                        ...
                                        {paymentType === 'upi' ? (
                                            <div className="flex flex-col md:flex-row gap-8">
                                                <div>
                                                    ...
                                                    <p className="font-mono font-black text-lg text-foreground tracking-tight underline decoration-authority-hr/30 underline-offset-4">
                                            ...
                                            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                                                ...
                                                <div>
                                                    ...
                                                    <p className="font-mono font-bold text-authority-hr">{payment.vendor_ifsc_code || 'N/A'}</p>
                                                </div>
                                                <div>
                                    ...
                                    <div className="space-y-4 pt-4 border-t border-border/50">
                                        <Textarea
                                            ...
                                            placeholder="Reason for rejection (e.g. Insufficient balance, Policy violation)..."
                                            className="bg-background/80 focus:ring-authority-hr/30 resize-none min-h-[100px]"
                                        />
                                        <div className="flex flex-col sm:flex-row gap-3">
                                            <Button
                                                onClick={() => handleVerify(payment.id)}
                                                disabled={isSaving}
                                                className="flex-1 h-12 rounded-xl bg-authority-hr hover:bg-authority-hr/90 font-black shadow-lg shadow-authority-hr/20 transition-all active:scale-95 translate-y-0 hover:-translate-y-1"
                                            >
                ...
                {requests.length === 0 && (
                    ...
                    >
                        <div className="w-20 h-20 rounded-full bg-authority-hr/10 flex items-center justify-center text-authority-hr/30">
                            <ShieldCheck className="w-10 h-10" />
                        </div>
                        <div>
                            <h3 className="text-xl font-bold text-foreground/80">Audit Queue Clear</h3>
                            <p className="text-muted-foreground max-w-xs mx-auto">All HR-related payment audits have been cleared.</p>
                        </div>
                    </motion.div>
                )}
            </div>
