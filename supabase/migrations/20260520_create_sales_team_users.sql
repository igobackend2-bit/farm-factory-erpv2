...
BEGIN
  ...
  IF NOT EXISTS (SELECT 1 FROM auth.users WHERE email = 'amirthalingam@farmersfactory.in') THEN
    uid := gen_random_uuid();
    INSERT INTO auth.users (
      id, instance_id, email, encrypted_password,
      email_confirmed_at, raw_app_meta_data, raw_user_meta_data,
    ...
    RAISE NOTICE 'Created: amirthalingam@farmersfactory.in';
  ELSE
    RAISE NOTICE 'Already exists: amirthalingam@farmersfactory.in';
  END IF;
...
-- ── Verify: show all created sales users ────────────────────
